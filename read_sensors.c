...
#define NUM_CHANNELS 5 // 4 channels for LDRs and 1 channel for solar pannel voltage
...
typedef struct {
    ...
    uint8_t adc_pins[NUM_CHANNELS];                        // ADC pins (e.g., GPIO_PIN_1, GPIO_PIN_2, etc.)
...
const ADC_Config adc_config = {
    ...
    .adc_pins = {GPIO_PIN_1, GPIO_PIN_2, GPIO_PIN_3, GPIO_PIN_4, GPIO_PIN_5}, // PE1, PE2, PE3, PE4, PE5
    .adc_channels = {ADC_CTL_CH2, ADC_CTL_CH1, ADC_CTL_CH0, ADC_CTL_CH9, ADC_CTL_CH8}, // AIN2, AIN1, AIN0, AIN9, AIN8
//    .adc_pins = {GPIO_PIN_1, GPIO_PIN_2}, // PE1, PE2
//    .adc_channels = {ADC_CTL_CH2, ADC_CTL_CH1}, // AIN2, AIN1
    .num_channels = 5,
...
volatile uint32_t adcValues[5];
volatile float input_voltage = 0.0;

volatile float ldr_resistance[NUM_CHANNELS-1] = {0, 0, 0, 0};
...
void config_ADC(void) {
    ...
    ADCSequenceStepConfigure(adc_config.adc_base, adc_config.sequencer, 4, adc_config.adc_channels[4] | ADC_CTL_IE | ADC_CTL_END); // stop sampling
...
void calculate_resistance(void) {
    ...
    for (i = 0; i < NUM_CHANNELS-1; i++) {
        float avg_adc_value = adcValues[i];
        float input_voltage = avg_adc_value * (3.3 / 4096.0);
        float ref_res = 10000.0;
        ldr_resistance[i] = ref_res * (input_voltage / 3.3);
    }
}
